...
grep -q 'classifier_should_skip' "$SKILLD"/*.md || {
...
for agent in code-reviewer security-auditor test-engineer performance-tuner spec-tracer; do
  grep -q "$agent" "$SKILLD"/*.md || { echo "FAIL: missing agent $agent in SKILL.md"; exit 1; }
done

# 7. Old phrase removed (negative assertion).
if grep -q "partial string match on the finding" "$SKILLD"/*.md; then
...
if ! grep -qE 'FINDINGS_JSON=\$\(mktemp\)|FINDINGS_JSON=' "$SKILLD"/*.md; then
...
if ! grep -qE 'REVIEW_FILE_NAME=' "$SKILLD"/*.md; then
...
if grep -q 'FINDINGS_LIST_PYTHON' "$SKILLD"/*.md; then
  # Variable still referenced — verify it has an assignment line before the heredoc.
  ASSIGN_LINE=$(grep -n 'FINDINGS_LIST_PYTHON=' "$SKILLD"/*.md 2>/dev/null | head -1 | cut -d: -f1 || echo "")
  HEREDOC_LINE=$(grep -n "<<'PYEOF'" "$SKILLD"/*.md | head -1 | cut -d: -f1)
...
grep -q 'two_axis=True' "$SKILLD"/*.md || {
...
grep -q 'CHARTER_SCOPE_CONFLICT' "$SKILLD"/*.md || {
...
grep -q 'Charter scope adjacent' "$SKILLD"/*.md || {
...
grep -q 'Two-axis classification' "$SKILLD"/*.md || {
