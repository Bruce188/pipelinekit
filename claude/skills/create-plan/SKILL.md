...
5. **Archive the active Status block (before rewriting it):**
   ...
   - **Iteration-boundary anchors (OQ-1):** the rotator recognises **both** `## Status` AND `## Iteration:` as iteration-boundary H2 markers. Either anchor is valid; the substep operates on whichever appears in `docs/progress.md`. Concretely:
     - If the file uses `## Status — <feature>` headers (current convention), extract rows between the active `## Status` H2 and the next H2 (`## Deferred` or `## Legend`).
     - If the file uses `## Iteration: <name>` headers (legacy/alt convention seen in consumer projects), extract the older `## Iteration:` blocks (i.e. all but the most recent) between each `## Iteration:` H2 and the next H2. The MOST RECENT `## Iteration:` block is preserved as the active block.
     - Files mixing both anchors are tolerated: treat each `## Status` OR `## Iteration:` H2 as a candidate boundary; the youngest block (by file order — last to appear) is the active one and is preserved; older blocks rotate.
   - Append each extracted block to `docs/archive/progress-v<N>.md` (or successive `progress-v<N+k>.md` if multiple older blocks exist, per Versioning Convention next-N numbering). Create `docs/archive/` and the file if absent. If the target file already exists (re-run case), append with a `---` separator and a timestamped comment so history is preserved.
   - **Backwards-compat:** consumer-project `progress.md` files with no recognised anchor (any layout) gracefully no-op — the rotator logs `rotator: no iteration-boundary H2 found, skipping archive` and continues. Never delete or rewrite anything when no anchor is recognised.
   - **Content preservation:** no `done` task rows are lost — rotation only moves the superseded iteration blocks; all completed history stays in `docs/progress.md` per the history-preserving rule in `~/.claude/rules/workflow.md` § Plan & Progress.
   - After extraction, `docs/progress.md` should retain: pointers, `## Current`, an empty `## Status` header row (table headers only, ready for new rows), the unchanged `## Deferred` table in its original position, and `## Legend`. Any `## Iteration:` block that was preserved as "active" is also retained verbatim.
