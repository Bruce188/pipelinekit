...
#### Agentmemory complementarity

The vendored `claude/lib/agentmemory/` skeleton enables a structured-retrieval layer over the same `~/.claude/memory/` flat-file system (semantic similarity, decay, contextual recall). AgentMemory is a community project (NOT Anthropic-official) — see `claude/lib/agentmemory/NOTICE.md` for vendor attribution.
...
4. **Default per-run behaviour unchanged.** When no agentmemory queries are issued the flat-file path is the only path; agentmemory is additive and inert until queried. No Phase Tool Routing or Memory Feed table changes.
