#!/usr/bin/env bash
# claude/lib/sandbox/providers/podman.sh — Rootless Podman sandbox provider.
#
# This provider is selected when SANDBOX_PROVIDER=podman (or SANDBOX_PROVIDER=auto
# and podman is on PATH).
#
# Public interface (implements the SandboxProvider contract):
#   sandbox_enter <worktree-path> <command...>   — run command inside a rootless podman container
#   sandbox_exit  <session-id>                    — no-op; --rm handles container cleanup
#
# Key behaviours:
#   - Bind-mounts the worktree directory READ-WRITE; NEVER copies worktree contents.
#     The worktree path inside the container matches the host path exactly, so all
#     relative-path references within the command continue to work unchanged.
#   - Composes with claude/hooks/env-scrub.py --prefix-args to build an env-scrub
#     argv prefix — do NOT redefine SCRUB_VARS here; env-scrub.py owns that list
#     and keeps it in sync with the Claude Code hook chain.
#   - Uses --rm so the container is removed automatically on exit; sandbox_exit is a no-op.
#   - Runs rootless (--userns=keep-id) so files written inside the container retain the
#     caller's UID/GID on the host filesystem, avoiding permission mismatches.
#   - Uses :Z SELinux relabelling on the bind mount. Safe to use on non-SELinux systems
#     (ignored when SELinux is disabled or permissive).
#
# PIPELINE_NO_SANDBOX=1: handled upstream by provider_detect() in SandboxProvider.sh;
#   this file is not sourced when that flag is set — worktree-only.sh is used instead.
#
# Env vars (all optional):
#   SANDBOX_PODMAN_IMAGE   — container image (default: localhost/pipelinekit-sandbox:latest)
#   CLAUDE_HOME            — path to the Claude Code config directory (default: $HOME/.claude)
#
# Runtime requirements:
#   - podman (rootless mode) must be installed and functional.
#     Detection is performed upstream in provider_detect(); if podman is absent when
#     SANDBOX_PROVIDER=auto was requested, the provider falls back to the next available
#     runtime (docker → worktree-only) without reaching this file.
#
# This file is sourced, never executed directly.

# ---------------------------------------------------------------------------
# sandbox_enter <worktree-path> <command...>
#
# Runs the given command inside a rootless Podman container with the worktree
# directory bind-mounted read-write.
#
# Arguments:
#   $1   — absolute path to the worktree directory (must exist on the host)
#   $@   — command and arguments to execute inside the container
#
# env-scrub.py --prefix-args emits one token per line: `env`, then alternating
# `-u VAR` pairs. The provider reads those tokens into a bash array and dispatches
# the user's command as exec-style argv (`"${prefix[@]}" "$@"`), preserving argv
# boundaries. Shell-string concatenation via `sh -c` is intentionally avoided —
# it would re-tokenize the command and expose a shell-injection surface to
# any caller passing AI-generated text with shell metacharacters.
# ---------------------------------------------------------------------------
sandbox_enter() {
  local wt="$1"; shift
  local image="${SANDBOX_PODMAN_IMAGE:-localhost/pipelinekit-sandbox:latest}"
  local claude_home="${CLAUDE_HOME:-$HOME/.claude}"
  local prefix_out prefix_rc
  local -a prefix

  # Capture env-scrub.py output via command substitution so we can observe its
  # exit status. `mapfile < <(...)` would silently discard producer failures
  # (mapfile returns 0 even when the producer exits non-zero) — that would
  # silently drop the scrub prefix and run the container env-bare. Fail closed
  # instead: if the helper fails or returns an unexpected first token, refuse
  # to launch the container.
  prefix_out=$(python3 "${claude_home}/hooks/env-scrub.py" --prefix-args)
  prefix_rc=$?
  if [ "$prefix_rc" -ne 0 ]; then
    echo "sandbox(podman): env-scrub.py --prefix-args failed (rc=$prefix_rc); refusing to launch container" >&2
    return "$prefix_rc"
  fi
  mapfile -t prefix <<< "$prefix_out"
  if [ "${#prefix[@]}" -lt 1 ] || [ "${prefix[0]}" != "env" ]; then
    echo "sandbox(podman): env-scrub.py --prefix-args returned unexpected output; refusing to launch container" >&2
    return 1
  fi

  podman run --rm \
    --userns=keep-id \
    --volume "$wt:$wt:rw,Z" \
    --workdir "$wt" \
    "$image" \
    "${prefix[@]}" "$@"
}

# ---------------------------------------------------------------------------
# sandbox_exit <session-id>
#
# No-op for the Podman provider.
#
# The --rm flag on `podman run` removes the container automatically when the
# command exits. No persistent session or named container is created, so there
# is nothing to clean up in this hook.
#
# Arguments:
#   $1   — session ID (ignored; present for interface compatibility)
# ---------------------------------------------------------------------------
sandbox_exit() {
  # --rm cleans up the container; nothing else to do.
  return 0
}
