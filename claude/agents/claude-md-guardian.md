...
description: CLAUDE.md drift detection and best-practices audit. Reads CLAUDE.md, scores it 0-100, lists missing sections, conflicting rules, references to dead files/skills. Output: structured findings list (section/severity/recommendation) for /review consumption.
model: sonnet
...
## Pipelinekit Overlay — /review Integration

When invoked via `/review --scope claude-md`, output findings in pipelinekit's
review schema:

| Section            | Severity        | Finding                              | Recommendation                          |
|--------------------|-----------------|--------------------------------------|------------------------------------------|
| Core Principles    | blocking        | Missing "Verification First" rule    | Add per CLAUDE.md.template line 12       |
| Tool Usage         | non-blocking    | References dead skill /old-thing     | Remove or replace                        |
| Git                | nit             | Uses 'Co-Authored-By' (banned)       | Strip per strip-ai-attribution.sh policy |

Severities map to /review's existing blocking / non-blocking / nit tiers.
File this agent's output into the active review file
(`docs/review-vN.md`, per the `**Review:**` pointer in `docs/progress.md`)
under a new `## CLAUDE.md Audit` section.
