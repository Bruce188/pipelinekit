#!/usr/bin/env bash
# hetzner-bootstrap.sh — idempotent provisioner for Hetzner Cloud CX22 (x86_64)
# Platform: x86_64 (Ubuntu 22.04 / 24.04 LTS), 2 vCPU / 4 GB RAM / 40 GB disk
#
# Usage:
#   bash hetzner-bootstrap.sh [--self-test] [--no-swap]
...
ARCH="$(uname -m)"
if [ "$ARCH" != "x86_64" ]; then
  die "This script targets Hetzner CX22 (x86_64). Detected arch: ${ARCH}. For Oracle Cloud ARM Ampere A1, use oracle-bootstrap.sh instead."
fi
...
# --------------------------------------------------------------------------- #
# Step 3: Idempotency guards (self-test stops here after swap detection)       #
# --------------------------------------------------------------------------- #
SWAP_EXISTS=false
...
if [ "$SELF_TEST" = "true" ]; then
  log "[self-test] arch=${ARCH}, swap_exists=${SWAP_EXISTS}, no_swap_flag=${NO_SWAP}, claude_exists=${CLAUDE_EXISTS}, repo_exists=${REPO_EXISTS}"
...
# --------------------------------------------------------------------------- #
# Step 4: Swap (mandatory before apt-get on 4 GB RAM to prevent npm OOM)      #
# CX22 has only 4 GB RAM; npm install for typescript-language-server can OOM  #
# without a swapfile. Default 2 GB. Use --no-swap to skip (not recommended).  #
# --------------------------------------------------------------------------- #
if [ "$NO_SWAP" = "true" ]; then
  log "WARNING: --no-swap passed. Swap skipped. npm may OOM on 4 GB RAM during LSP install."
else
  if [ "$SWAP_EXISTS" = "true" ]; then
    log "Swap already active at /swapfile — skipping swapfile creation"
  else
    log "Creating 2 GB swapfile at /swapfile (required before apt-get on 4 GB RAM)"
...
# --------------------------------------------------------------------------- #
# Step 6: Node 20 LTS via NodeSource (x86_64 distribution)                   #
# --------------------------------------------------------------------------- #
...
bash scripts/verify.sh || log "verify.sh exited non-zero — check ${LOG_FILE} for details"
