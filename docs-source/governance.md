...
- **Hook block + error rate.** Count of `exit 2` (blocking — the gate refused the tool call and fed its reason back to Claude) and `exit 1` (non-blocking hook error surfaced to the user; the tool call still proceeds) outcomes per week, read from `~/.claude/logs/hook-events.jsonl` if present, otherwise from grep against the session logs. A spiking `exit 2` rate on a single gate signals a convention the team keeps hitting — friction or a too-strict rule; a spiking `exit 1` rate signals a recently introduced hook bug. Target: aggregate non-zero hook exit rate stays under 5% of total hook invocations per week.
